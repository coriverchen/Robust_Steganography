function TestRobustnessUR_JUNIWARD(precover_dir,cover_dir,stego_dir,attack_QF,payload,T,beta)
% 检验向上鲁棒隐写的隐写正确率
...
for i_img = 1:cover_num
    ...
    nzAC = nnz(PC_COEFFS) - nnz(PC_COEFFS(1:8:end,1:8:end));
     ...
     % 对图像进行预处理，产生最终载体
    [VulnerableBlock] = PreprocessUR(precover_Path,cover_Path,T);
...
%     rs_encoded_msg = raw_msg;
    ...
    % 根据鲁棒性调制失真
    [rho_p,rho_m] = CostUR(rho1_P,rho1_M,VulnerableBlock,cover_Path,0,beta);
...
ave_0error_rate = (files_num-nnz(bit_error_rate))/files_num;

fprintf('%s\n',['payload: ',num2str(payload),'  ave_error_rate: ',num2str(ave_error_rate),'  ave_0error_rate: ',num2str(ave_0error_rate),'  files_num: ',num2str(files_num)]);
